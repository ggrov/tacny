...
// the "tactic" keyword was used when declaring a tactic stmt
bool IsTacticKeyword() {
    return oldTok != null && oldTok.kind == _tactic;
}
			...
			if (guardEllipsis != null || bodyEllipsis != null) {
			 ...
			 if (body == null && !isDirtyLoop) {
			   body = new BlockStmt(x, endTok, new List<Statement>());
			    ...
			    stmt = new WhileStmt(x, endTok, guard, invariants, new Specification<Expression>(decreases, decAttrs),
			        new Specification<FrameExpression>(mod, modAttrs), body) {TacAps = tacAps};
			}
