...
class Main {
  method RecursiveMark(root: Node, ghost S: set<Node>)
    requires root in S
    requires forall n :: n in S ==> n != null && forall ch :: ch in n.children ==> ch == null || ch in S
  ...
  method RecursiveMarkWorker(root: Node, ghost S: set<Node>, ghost stackNodes: set<Node>)
    ...
    requires forall n: Node :: n in S ==> n != null && forall ch: Node :: ch in n.children ==> ch == null || ch in S
    requires forall n: Node :: n in S && n.marked ==> n in stackNodes || forall ch: Node :: ch in n.children && ch != null ==> ch.marked
    requires forall n: Node :: n in stackNodes ==> n != null && n.marked
    modifies S
    ensures root.marked
    ensures forall n: Node :: n in S && n.marked ==> n in stackNodes || forall ch: Node :: ch in n.children && ch != null ==> ch.marked
    ensures forall n: Node :: n in S && old(n.marked) ==> n.marked
    ensures forall n: Node :: n in S ==> n.childrenVisited == old(n.childrenVisited) && n.children == old(n.children)
    ...
    ensures forall n :: n in S ==> n.childrenVisited == old(n.childrenVisited) && n.children == old(n.children)
  {
    ...
    while true
      ...
      invariant !fresh(path)
      invariant old(ReachableVia(root, path, t, S))
      invariant forall n, pth :: n in S && n.marked && pth == n.pathFromRoot ==> !fresh(pth)
      invariant forall n, pth :: n in S && n.marked && pth == n.pathFromRoot ==> old(ReachableVia(root, pth, n, S))
      ...
      invariant forall k :: 0 < k < |stackNodes| ==> stackNodes[k].children[stackNodes[k].childrenVisited] == stackNodes[k - 1]
      invariant forall k :: 0 <= k && k + 1 < |stackNodes| ==> old(stackNodes[k].children)[stackNodes[k].childrenVisited] == stackNodes[k + 1]
