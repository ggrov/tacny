...
function PickLargest(s: set<int>): int
  requires s != {}
{
  LargestElementExists(s); var x :| x in s && forall y :: y in s ==> y <= x; x
}
  ...
  ensures exists x :: x in s && forall y :: y in s ==> y <= x
{
  var s' := s;
  while true
    invariant s' != {} && s' <= s     invariant forall x, y :: x in s' && y in s - s' ==> y <= x   
  variant();
  {
    var x :| x in s';
...
{
  solved {
    var f :| f in merge(variables(), params());
    add_variant(f) || {
      var y :| y in merge(variables(), params()) && f != y;
      add_variant(f - y);
    };
  }
}