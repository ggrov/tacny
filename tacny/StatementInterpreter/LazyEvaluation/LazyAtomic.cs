...
namespace LazyTacny {
    ...
    public class Atomic {
        ...
        public static Expression ResolveTacticFunction(ITactic tac, UpdateStmt tac_call, MemberDecl md, Tacny.Program tacnyProgram, List<IVariable> variables, List<IVariable> resolved) {
            ...
            foreach (var result in atomic.ResolveTacticFunction2(expt))
                return result.TreeToExpression();
        ...
        protected IEnumerable<ExpressionTree> ResolveTacticFunction2(ExpressionTree body) {
            Contract.Requires(body != null);

            var leafs = body.GetLeafs();
            var leafResolvers = new Dictionary<ExpressionTree, IEnumerable<object>>();

            foreach (var leaf in leafs)
                leafResolvers.Add(leaf, ProcessStmtArgument(leaf.data));
        ...
        protected IEnumerable<ExpressionTree> GenerateExpressionTree(ExpressionTree tree, Dictionary<ExpressionTree, IEnumerable<object>> leafResolvers) {
            ...
            Util.Printer.P.GetConsolePrinter().PrintExpression(tree.TreeToExpression(), false);
            ...
            foreach (var value in kvp.Value) {
                var newLeaf = kvp.Key.CopyNode();
                newLeaf.data = value as Expression;
        ...
        protected void ResolveTacticFunction(ExpressionTree body) {
            Contract.Requires(body != null);
            if (body.IsLeaf()) {
                if (body.data is ApplySuffix) {
                    var type = StatementRegister.GetAtomicType(body.data as ApplySuffix);
                    if (type != StatementRegister.Atomic.UNDEFINED) {
                        foreach (var solution in ProcessStmtArgument(body.data)) {
                            var newNode = body.Copy();
                            newNode.data = solution as Expression;
                            var qq = ExpressionTree.FindAndReplaceNode(body.root, newNode, body);

                            break;
                        }
                    } else {
                        var aps = body.data as ApplySuffix;
                        if (aps.Lhs is ExprDotName) {
                            foreach (var solution in ProcessStmtArgument(aps.Lhs)) {
                                body.data = new ApplySuffix(aps.tok, solution as Expression, aps.Args);
                                break;
                            }
                        }

                    }
                } else if (body.data is FunctionCallExpr) {
                    var qq = body.data as FunctionCallExpr;
                } else {
                    foreach (var solution in ProcessStmtArgument(body.data)) {
                        body.data = solution as Expression;
                        break;
                    }
                }

            } else {
                ResolveTacticFunction(body.lChild);
                if (body.rChild != null)
                    ResolveTacticFunction(body.rChild);
            }
        }
