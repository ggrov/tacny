...
// Command Line Options: /rprint:- /contracts:0 invar_tac_test.dfy
// main_program_id
...
class efault {
  ...
  lemma aux_equiv(list: List<int>, x: int)
    ensures equiv(Cons(x, list), aux(x, list))
  {
    match list
    case Nil =>
    case Cons(head, tail) =>
    ...
    ensures sorted(aux(x, l))
 {
  ...
  {
    existence_proof(l); var l' :| equiv(l, l') && sorted(l'); l'
  }
    ...
    ensures equiv(l, Sort(l)) && sorted(Sort(l))
  {
    ...
    case Cons(x, m) =>
      var m' := Sort(m);
      assert equiv(l, Cons(x, m')); aux_equiv(m', x); aux_sorted(m', x); aux(x, m')
  }
